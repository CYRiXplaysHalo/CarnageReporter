...
if not os.path.exists(dir_path + "\\" + "config.ini"):
    ...
    tag = input("Enter gamertag which you want to collect stats for: ").lower()
        ...
        for line in config:
            if 'gamertag: ' in line:
                ...
                if input("Current gamertag = " + tag + "\nDo you want to make a change (Y/N)? ").lower().strip()[:1] != "y":
                    ...
                    tag = input(
                        "Enter gamertag which you want to collect stats for: ").lower()
                    f = open(dir_path + "\\" + "config.ini", "w")
                    f.write("gamertag: " + str(tag))
                    print("Loading...")
                    time.sleep(3)
                    ...
                    try:
                        ...
                        if latest_mcc in file:
                            ...
                            if csv == 1:
                                ...
                                # Creates a table to store all the game data
                                meta = MetaData()

                                carnage_report = Table(
                                    'students', meta,
